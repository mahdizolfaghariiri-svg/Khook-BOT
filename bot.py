...
TOKEN = os.environ.get("BOT_TOKEN", "")          # توکن بات از BotFather
ADMIN_ID = int(os.environ.get("ADMIN_ID", "0"))  # آیدی عددی مدیر بازی
PORT = int(os.environ.get("PORT", "8000"))       # پورتی که Render به ما می‌دهد
...
def send_to_group(context, text):
    if not data.get("group_id"):
        return False
    try:
        context.bot.send_message(chat_id=data["group_id"], text=text)
        return True
...
async def forward_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text:
        return
    ok = send_to_group(context, f"{player_label(p)}:\n{text}\n\n— ارسال ناشناس")
    if ok:
...
async def khook(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text:
        ...
        return
    ok = send_to_group(context, f"🐷 {text}\n\n— خوک")
